...
spike patterns corresponding to the ones shown
in the original Mihalas-Niebur paper.
The spike_classifier with NNI-optimized parameters
and pre-trained weights is used.

SPIKING ACTIVITY REPRODUCED FROM THE MN PAPER.
...
data_filepath = "./data/mn_classes_original_10_repetitions_data.pkl"
label_filepath = "./data/mn_classes_original_10_repetitions_labels.pkl"

infile = open(data_filepath, "rb")
encoded_data = pkl.load(infile)
infile.close()
spikes = np.array(encoded_data)[:,0].copy()
for num, el in enumerate(spikes):
    spikes[num] = np.array(el)
...
data = spikes.tolist()
label = value2index(encoded_label, labels_mapping)
lbl_string = np.unique(encoded_label).tolist()
...
log_path = os.path.join("./logs/activity_classification/MN_activity",experiment_name)
...
LOG.debug("Original MN patterns classification\n")
...
### GPU
# NOTE that for the original patterns, GPU is not really needed
if settings["use_gpu"]:
...
parameters_path = "./parameters/optimized/spike_classifier/mn_classes/parameters.json"
...
def classify_spikes(input_spikes, single_input, labels_mapping, trained_path, device=device):
    ...
    spks_out, _, _ = run_snn(inputs=single_sample, nb_steps=input_spikes.shape[0], layers=layers)
...
for num,el in enumerate(data):
    ...
    if len(el.shape) > 1:
        ...
        assert len(el.shape) == 2, "Check the input activity is 1D."

    if np.count_nonzero(el.flatten()) > 0:
        n_spikes.append(int(el.sum().item()))
        sparsity.append(np.round(1-el.mean().item(),4))
        non_zero_channels += 1
        LOG.debug("Input {}/{} from original patterns reproduced:".format(num+1,len(data)))
        LOG.debug("Behaviour: {}".format(list(labels_mapping.keys())[label[num]]))
        pred, probs = classify_spikes(activity_spikes, True, labels_mapping, trained_layers_path)
        original.append(list(labels_mapping.keys())[label[num]])
        behaviour.append(pred)
        behaviour_probs.append(np.round(probs.numpy()*100,2))
        LOG.debug("Behaviour prediction: {} ({})".format(pred, labels_mapping[pred]))
        LOG.debug("Label probabilities (%): {}\n".format(np.round(np.array(probs.numpy())*100,2)))
...
activity_classification["Original"] = original
...
df2pkl_path = os.path.join("./results/activity_classification/MN_activity",experiment_name)
create_directory(df2pkl_path)
activity_classification.to_pickle(os.path.join(df2pkl_path,f"{experiment_datetime}.pkl"))
...
grouped = activity_classification[["Original","Probabilities"]].groupby("Original", as_index=False).mean()
classified_activity_df = pd.DataFrame(index=range(len(lbl_string)), columns=range(len(list(labels_mapping.values()))))
for ii in range(len(labels_mapping)):
    for jj in range(len(list(labels_mapping.keys()))):
        classified_activity_df.iloc[ii,jj] = float(grouped[grouped["Original"]==list(labels_mapping.keys())[ii]]["Probabilities"].item()[-1][jj])
...
plt.xlabel("Original MN pattern")
plt.ylabel("Behaviour probability (%)")
plt.xticks(ticks=[ii+0.5 for ii in range(len(labels_mapping))],labels=labels_mapping.keys(), rotation=0)
plt.yticks(ticks=[ii+0.5 for ii in range(len(labels_mapping))],labels=labels_mapping.keys(), rotation=0)
plt.tight_layout()
if settings["save_hm"]:
    path_for_plots = os.path.join("./results/plots/activity_classification/MN_activity",experiment_name,f"{experiment_datetime}")
