...
def train(dataset, lr=0.0015, nb_epochs=300, opt_parameters=None, layers=None, dataset_test=None, seed=42, use_trainable_tc=False, use_trainable_out=False, use_dropout=False):
...
def build_and_train(device, data_steps, dt, ds_train, ds_test, epochs, labels, lr, seed, use_trainable_tc=False, use_trainable_out=False, use_dropout=False):
    ...
    loss_hist, accs_hist, best_layers = train(
        ds_train, lr=lr, nb_epochs=epochs, opt_parameters=opt_parameters, layers=layers, dataset_test=ds_test, seed=seed, use_trainable_tc=False, use_trainable_out=False, use_dropout=False)
...
def ConfusionMatrix(dataset, save, layers, labels):
    ...
    print("Accuracy from Confusion Matrix: {} +- {}".format(np.mean(accs), np.std(accs)))
    ...
    if save:
        ...
        if use_trainable_out:
            path_to_save_fig = path_to_save_fig + "_train_out"
        if use_dropout:
            path_to_save_fig = path_to_save_fig + "_dropout"
        path_to_save_fig = path_to_save_fig + "_cm.png"
...
def NetworkActivity(dataset, save, layers, labels):
    ...
    if save:
        ...
        if use_dropout:
            path_to_save_fig = path_to_save_fig + "_dropout"
        path_to_save_fig = path_to_save_fig + "_hidden_layer.png"
        plt.savefig(path_to_save_fig, dpi=300)
    ...
    if save:
        ...
        if use_dropout:
            path_to_save_fig = path_to_save_fig + "_dropout"
        path_to_save_fig = path_to_save_fig + "_output_layer.png"
        plt.savefig(path_to_save_fig, dpi=300)
    else:
        plt.show()
