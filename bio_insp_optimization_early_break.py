...
def train(dataset, lr=0.0015, nb_epochs=300, opt_parameters=None, layers=None, dataset_test=None, break_early=False, patience=None):
    ...
    for e in range(nb_epochs):
        ...
        if break_early:
            delta_test_acc_trace[trace_counter] = test_acc - delta_test_acc_trace[trace_counter-1]
            if trace_counter == patience:
                trace_counter = 0
            else:
                trace_counter += 1
            if e >= patience:
                # stop traning if acc starts to drop of
...
for generation in range(generations):
    ...
    for identifier, individual in enumerate(population_list):
        ...
        loss_hist, acc_hist, best_layers = build_and_train(
            data_steps, ds_train, ds_test, epochs=epochs, break_early=True, patience=5)
