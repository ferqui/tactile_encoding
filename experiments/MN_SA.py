...
def main():
    ...
    # ds_train_max = ds_train.tensors[0].max()
    # ds_test_max = ds_test.tensors[0].max()
    print('ds_test_min',ds_test.tensors[0].min())
    print('ds_train_min', ds_train.tensors[0].min())
    ds_max = float(torch.Tensor([ds_test.tensors[0].max(),ds_train.tensors[0].max()]).max())
    ds_min = float(torch.Tensor([ds_test.tensors[0].min(),ds_train.tensors[0].min()]).min())
    ...
    nn.init.normal_(
        A1, mean=MNparams_dict['A2B'][1], std=fwd_weight_scale / np.sqrt(nb_inputs))

    A2 = torch.empty((nb_inputs,))
    nn.init.normal_(
        A2, mean=MNparams_dict['A2B'][2], std=fwd_weight_scale / np.sqrt(nb_inputs))

    network = nn.Sequential(Encoder(nb_inputs, encoder_weight_scale, nb_input_copies,ds_min,ds_max),
                            MN_neuron(nb_inputs, a, A1, A2, train=False),
    ...
    for e in pbar:
        ...
        for x_local, y_local in dl_train:
            ...
            ## Regularizer commented for this experiment!!!
