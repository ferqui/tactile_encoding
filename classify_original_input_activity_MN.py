...
spike patterns obtained with original current
input as in the MN paper. The spike_classifier 
...
parser.add_argument('-experiment_name',
                    type=str,
                    default="original_input_activity_classifier",
...
log_path = "./logs/classification/original_input_activity_MN"
...
class MN_neuron_original_input(nn.Module):
    NeuronState = namedtuple('NeuronState', ['V', 'i1', 'i2', 'Thr', 'spk'])

    def __init__(self, nb_inputs, parameters_combination, dt=1 / 1000,
                 a=2.743, A1=0.03712, A2=-0.5089, b=11.4, G=47.02, k1=200, k2=20, R1=0, R2=1,
                 C=1, train=False):  # default combination: optimized from training for Braille classification (except for train)
        super(MN_neuron_original_input, self).__init__()
...
if save_fig:
    path_for_plots = "./results/plots/classification/original_input_activity_MN"
    create_directory(path_for_plots)
print("*** classification started ***")
for num,el in enumerate(original_input_data):
    encoder_MN = MN_neuron_original_input(nb_inputs=1, parameters_combination=None).to(device)
